...
if [[ "${start_servers}" == "1" ]]; then
  for i in "${!suites[@]}"; do
    ...
    CUDA_VISIBLE_DEVICES="${gpu}" PYTHONUNBUFFERED=1 \
      ...
      bash "${STARVLA_DIR}/examples/simBenchmarks/LIBERO-plus/eval_files/run_policy_server.sh" \
      > "${server_log}" 2>&1 &
    server_pids+=("$!")
  done
...
for i in "${!suites[@]}"; do
  ...
  CUDA_VISIBLE_DEVICES="${gpu}" PYTHONUNBUFFERED=1 \
    "${LIBERO_Python}" "${STARVLA_DIR}/examples/simBenchmarks/LIBERO-plus/eval_files/eval_libero.py" \
    --args.host "${host}" \
    --args.port "${port}" \
...
LOG_DIR="${log_dir}" "${LIBERO_Python}" "${STARVLA_DIR}/examples/simBenchmarks/LIBERO-plus/eval_files/aggregate_results.py"
