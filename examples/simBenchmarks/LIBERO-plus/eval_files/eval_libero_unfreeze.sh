...
host="${host:-127.0.0.1}"
your_ckpt="${your_ckpt:-/home/xhy/starVLA/playground/Checkpoints/libero4in1_qwenpi_v3_unfreeze/checkpoints/steps_160000_pytorch_model.pt}"
...
category_fraction="${category_fraction:-0.5}"#默认1.0，即所有类别都评估
...
if [[ "${start_servers}" == "1" ]]; then
  for i in "${!suites[@]}"; do
    ...
    CUDA_VISIBLE_DEVICES="${gpu}"       your_ckpt="${your_ckpt}"       base_port="${port}"       gpu_id="${gpu}"       ABot_python="${ABot_python}"       PYTHONPATH="${STARVLA_DIR}:${PYTHONPATH:-}"       bash "${STARVLA_DIR}/examples/LIBERO-plus/eval_files/run_policy_server.sh"       > "${server_log}" 2>&1 &
    server_pids+=("$!")
  done
...
for i in "${!suites[@]}"; do
  ...
  CUDA_VISIBLE_DEVICES="${gpu}"     "${LIBERO_Python}" "${STARVLA_DIR}/examples/LIBERO-plus/eval_files/eval_libero.py"     --args.host "${host}"     --args.port "${port}"     --args.task-suite-name "${suite}"     --args.num-trials-per-task "${num_trials_per_task}"     --args.start-idx 0     --args.end-idx -1     --args.video-out-path "${suite_video_dir}"     --args.log-path "${log_dir}"     > "${eval_log}" 2>&1 &
  eval_pids+=("$!")
done
